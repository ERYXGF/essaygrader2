...
│   └── main.py                    # four-step orchestrator
│
├── tests/
│   └── test_plagiarism_checker.py # unit tests (no API calls)
│
...
- The cache is fully automatic — no flags to set.
...
- **Editing `config/essay_prompt.txt` regrades everything automatically** — the
  cache stores a hash of the rubric and invalidates itself when it changes.

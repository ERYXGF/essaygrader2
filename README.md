...
6. Writes a four-sheet Excel report to `output/ai_essay_grading_report.xlsx`
...
│
├── output/
│   ├── ai_essay_grading_report.xlsx          # generated by each run
...
# 6. Open output/ai_essay_grading_report.xlsx
```
...
It is read for one thing — **the submission date** (`Created`). That date cannot
be recovered from the PDFs: filesystem mtimes are reset by copying, and a PDF's
internal `CreationDate` records when the document was *authored*, so a candidate
reusing an old file would look like an older applicant than they are.
